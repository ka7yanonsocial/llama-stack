...
async def convert_message_to_openai_dict_new(
    ...
    async def _convert_message_content(
        ...
        async def impl(content: InterleavedContent):
            ...
            if isinstance(content, str):
                ...
                return [await impl(item) for item in content]
            else:
...
def convert_tooldef_to_openai_tool(tool: ToolDefinition) -> dict:
    ...
    if tool.parameters:
        ...
        for param_name, param in tool.parameters.items():
            properties[param_name] = {"type": TYPEHINT_TO_LITELLM_TYPE.get(param.param_type, param.param_type)}
